...
void write_test(char **keys_buf, char ***data_buf, int elem_num[__TEST_MAX_KEY__], int k, int *errors){
    int temp_flag = -1;
    char *temp;
    printf("-----------Attempting Writes ROUND %d/%d-----------", k+1, __TEST_MAX_POD_ENTRY__);
...
void read_test(char **keys_buf, char ***data_buf, int elem_num[__TEST_MAX_KEY__], 
    ...
    int temp_flag = -1;
    char *temp;
    printf("-----------Attempting Reads ROUND %d/%d-----------", k+1, __TEST_MAX_POD_ENTRY__);
    for(int i = 0; i < __TEST_MAX_KEY__; i++){
        temp = kv_store_read(keys_buf[i]);
        if(temp == NULL){
    ...
    if(read_all == NULL){
        ...
        *errors += 1;
        ...
        if(total_read != pattern_length){
            printf("Invalid Read Length Based on Pattern Analysis\r");
...
int main(){
    ...
    if(temp != NULL){
        errors++;
        printf("\nDid not return null on invalid read key. \n");
        free(temp);
    }
    temp_all = kv_store_read_all("need that ryzen");
    if(temp_all != NULL){
        errors++;
        printf("\nDid not return null on invalid read all key. \n");
        free(temp_all);
    }

    printf("\n-----------Attempting Simple Read and Writes-----------\n");
    for(int i = 0; i < __TEST_MAX_KEY__; i++){
    ...
    printf("\n-----------Testing FIFO/Read Order/Read Alls -----------\n");
    for(int i = 0; i < __TEST_MAX_KEY__; i++){
